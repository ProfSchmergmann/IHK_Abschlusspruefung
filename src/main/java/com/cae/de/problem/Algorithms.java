...
public class Algorithms {
	...
	public static AutoKorrelationsFunktion solve(Data data) {
		...
		var res = Algorithms.alg4(pair.value(), oE);
	...
	private static Pair<double[], double[]> alg1(int[] xStart, int[] yStart) {
	  ...
	  for (var i = 0; i < xStart.length; i++) {
	    xDach[i] = Math.abs((alpha * xStart[i]) - 132.3);
	 ...
	 */
	private static Pair<Float, Pair<Integer, Integer>> alg4(double[] yNorm, double[] oE) {
	  ...
	  return new Pair<>((float) (oE[indexR] - oE[indexL]), new Pair<>(indexL, indexR));
	}
	...
	/**
	 * Methode zur Berechnung des Maximums eines Double-Arrays. Es wird eine foreach Schleife intern
	 * genutzt, da ein Stream wahrscheinlich zu viel Overhead produzieren würde.
	 *
	 * @param array das Eingabe-Array
	 * @return das Maximum des Eingabe-Arrays
